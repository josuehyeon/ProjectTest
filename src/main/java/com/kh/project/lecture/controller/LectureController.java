...
public class LectureController {
	...
	public RegLectureSelectBoxVO selectCollAjax(Model model, CollegeVO collegeVO, MultipartHttpServletRequest multi) {
		//첨부파일 upload---------
		...
		//첨부될 폴더 지정
		String uploadPath ="C:\\Users\\조현재\\Documents\\workspaceSTS\\Shop\\src\\main\\webapp\\resources\\images\\";
		
		
		//모든 첨부파일 정보가 들어갈 공간 
		List<PdfVO> pdfVOList = new ArrayList<>();
		
		/*이건다음에 
		 * Map<String, String> fileNameMap = new HashMap<>(); fileNameMap.put("", "");
		 */
		...
		while (inputNames.hasNext()) { /* input태그 file속성 갯수만큼 돈다 */
			...
			try {
				...
				//단일 첨부
					...
					pdfVO.setPdfCode("IMG_"+String.format("%03d", nextPdfCode++));
